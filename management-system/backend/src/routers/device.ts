...
export const DeviceRouter = (deviceCollectionWs: DeviceCollection) =>
  tRPC.router({
    ...
    DownloadFile: tRPC.ProtectedProcedure.input(v.object({ device_id: UUID, file_name: v.string() })).query(
      async ({ ctx, input }) => {
        const connection = deviceCollectionWs.getDeviceConnection(input.device_id);
        if (!connection) return null;
    ...
    UploadFile: tRPC.ProtectedProcedure.input(
      v.object({ device_id: UUID, file_name: v.string(), file_data: v.any() }),

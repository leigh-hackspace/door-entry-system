...
async function serveAsset(assetDir: string, version: string, url: URL) {
  ...
  if (requestVersion && version !== requestVersion) {
    return new Response(`${version} !== ${requestVersion}`, { ...BASE_HEADERS, headers: { "Content-Type": "text/plain" } });
  }
  ...
  return new Response(body, { headers: { ...BASE_HEADERS, "Content-Type": mimeType, "Content-Length": String(stats.size) } });
}
...
async function serveHtmlFile(assetDir: string, version: string) {
  ...
  return new Response(html, { headers: { ...BASE_HEADERS, "Content-Type": "text/html", "Content-Length": String(html.length) } });
}
